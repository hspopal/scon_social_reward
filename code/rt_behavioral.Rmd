...
```{r}
group <- paste0(rt_data_mod$Run, rt_data_mod_typ$Condition)
group <- aggregate(model.matrix(model_run_peer) ~ group, FUN=mean)
# group
...
```{r}
group <- paste0(rt_data_mod_typ$Run, rt_data_mod_typ$Condition)
group <- aggregate(model.matrix(model_run_peer_typ) ~ group, FUN=mean)
# group
rownames(group) <- group$group
(group <- group[,-1])
kable(head(group))
...
```{r}
emm_mod <- emmeans(model_run_peer_typ, "Condition", var="Run")
emm_mod
```

```{r}
(contr_mat <- coef(pairs(emm_mod))[, c("c.2")])
```

```{r}
emmeans(model_run_peer_typ, ~ Condition*Run, contr = contr_mat, adjust='none')
```
