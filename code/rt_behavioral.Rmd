...
```{r}
model_run_peer <- lmer(Correct_RT_logz ~ Run*Condition + (1 + Run | participant_id) + (1 + Condition | participant_id),
                       data = rt_data_mod)
...
## Previous Peer Feedback and Run Predictin Reaction Time

```{r}
model_run_prev_val <- lmer(Correct_RT_logz ~ Run*Valence_prev + (1 + Run | participant_id) + (1 + Valence_prev | participant_id),
                       data = rt_data_mod)
...
rt_data_mod %>%
  ...
  summarise_at(vars(Correct_RT_logz), list(name = mean))
```
...
```{r}
r4_peer_diff <- rt_data <- data.frame(matrix(ncol = 2, nrow = 0))
...
print(paste(prop_learn, '% of participants had faster RTs for similar peers than dissimilar peers for their last run.'))
```
...
### Do participants show a decrease in RT after receiving negative feedback?
...
```{r}
model_run_peer_typ <- lmer(Correct_RT_logz ~ Run*Condition + (1 + Run | participant_id ) + (1 + Condition | participant_id ), data = rt_data_mod_typ)
summary(model_run_peer_typ)
```
In non-autistic adolescents, there a significant main effect of run, indicating that participants did get faster at responding as the task went on (b = -0.13, p < .001). There were two significant interactions. Non-autistic adolescents had significantly faster reaction times as the task went on for the dissimilar peer (B = -0.08, p = .018) and the similar peer (B = -0.07, p = .0361) conditions  compared to the control condition. 
...
Maybe try doing contrasts with a [matrix](https://tdbianco.netlify.app/post/contrasts-in-multilevel-mixed-models-with-an-interaction/)
