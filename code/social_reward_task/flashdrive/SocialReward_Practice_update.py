...
#
#                                ###DOCUMENTATION###
#    As of 11/16/21, the preFixDur is set to 10 seconds, while the postFixDur is set to 10 seconds.
...
dlg = gui.DlgFromDict(expInfo, title= '', fixed=['date'], order=['Participant ID'])
...
run = "1"
if subID == "":
    raise ValueError ("Enter'SocialReward_[subID]'. For example, SCN_104.")
...
os.chdir(cwd)
### If this is run 1, set the trial order for the participant.##
tmp = "SR_exp_stimuli" + "_" + subID + "_practice.csv"
    ...
    for j in range(0,8):
        if j < 6:
            if j%2==1:
                ...
                out.loc[j,'ItemNumber'] = AnswerSheet.loc[cnt_pos,'pos_practice']
                print(out.loc[j,'ItemNumber'])
                out.loc[j,"ItemText"] = MasterSheet.loc[out.loc[j,'ItemNumber']-1,"Item_pos"]
                cnt_pos += 1
            else:
                out.loc[j,'ItemNumber'] = AnswerSheet.loc[cnt_neg,'neg_practice'] 
                print(AnswerSheet.loc[cnt_neg,'neg_practice'])
...
fileName = "SR_%s_practice.csv" %(subID)
...
# print(outfile)
    ...
    #else:
        #print("Overwriting existing log file") # raise ValueError ("A log file already exists for this run")
...
os.chdir(cwd)
stim_df = pd.read_csv(out_csv, nrows = 8)
...
## Button interface
# scanner_coms = ScannerComs(port='COM3', timeout=0.001, baudrate=19200) # real
scanner_coms = ScannerComs(port='COM4', timeout=0.001, baudrate=19200) # mock
...
#        message74.setAutoDraw(True)
    ...
    message72.setAutoDraw(True)
    message73.setAutoDraw(True)
    scanner_coms.wait_for_message(5)
    if event.getKeys(keyList=["escape"]):
        win.close()
        core.quit()
    ...
    while (responseTimer.getTime() < buttonDur) & button_flag:
        ...
        if FirstKey=="N/A" and len(answer) > 0 and responseTimer.getTime() > 0.3:  # first keypress and make sure it happens after the answer
            ...
            if FirstKey == 1 and left_or_right == "L":
                ...
                Answer_correctRT = Answer_firstRT  # correct RT = first RT
                correctbuttonPressTime = firstbuttonPressTime
                ChildFirstResponse = "R"  # Participant answered left
                rightAnswer.draw()
                rightCircle.draw()
                button_flag = 0;
        elif FirstKey !="N/A" and len(answer) > 0:  # if the first button press is wrong
            if answer[0] == 1 and left_or_right == "L":
