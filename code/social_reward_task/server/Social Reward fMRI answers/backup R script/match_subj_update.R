...
} else ############### anomaly detected, not enough pos/neg items
{
  if (length(idx_pos) < 69)
  {
    ...
    if (length(idx_pos) < 69)
    {
      pos_practice <- min(round(8 * pos_ratio), pos_left) # make sure now rounding error
    } else
    { 
...
if (matching_success==1) 
  {
    ...
    } else # if a insufficient pos or neg trials, need to reorganize the trials and add negative signs after moving them 
    {
      ...
      if (length(idx_pos)<69) # move neg items to pos
      {
        ...
        if (trials2move!=0) 
        {
          participant_answer$disagree_pos_comp[(trials$disagree_pos_comp+1):11] <- -1 * participant_answer$disagree_neg_comp[12:(11+trials2move)]
          participant_answer$disagree_neg_comp[11:(11+trials2move)] <- NA
          
          participant_answer$agree_pos_comp[(trials$agree_pos_comp+1):11] <- -1 * participant_answer$agree_neg_comp[12:(11+trials2move)]
          participant_answer$agree_neg_comp[11:(11+trials2move)] <- NA
