...
then
   ...
   # this is useful when a subject's scan is missing or incomplete (e.g., T1 scan) on MNC server, but the scanner still has that data
	...
	local_fmriprep=$(ls -d ../fmriprep_out/fmriprep/sub-SCN* | sed 's/[^0-9A-Z]*//g' | cut -c 4-) # get subject index, sed command reduces to SCN###, cut command reduces to just ID# - MK
	...
	for el in ${sub[@]}; do
		...
		then 
			...
			if [[ $local_dicom == *${idx}* ]]
			then
				echo SCN_$idx dicoms exist # skip downloading if the file has already been downloaded
				#if $rerun_fmriprep
				#then
					# Rerun fmriprep
			...
			# Check to see if fmriprep data exists
			if [[ $local_fmriprep == *${idx}* ]]
			then
				echo SCN_$idx fmriprep exist 
			else
				# submit sbatch on bswift
				ssh ${uname}@login.bswift.umd.edu "sbatch --export=indir="$SCN_bswift"/fmriprep,uname="$uname",subID="$idx" --job-name=SCN"$idx" --mail-user="${uname}"@umd.edu --output="$SCN_bswift"/log/sub-SCN"$idx".log /data/bswift-1/mkiely/SCN/code/fmriprep_SCN.sh"
			fi
