	...
	for el in ${sub[@]}; do
		...
		then 
			...
			then
				...
				download_data SCN"${idx}"
				echo SCN_${idx} download completes and start BIDS conversion
				./BidsConvert_SCN.sh BidsConvertParameters_SCN.sh SCN"${idx}" # BidsConvert_SCN.sh converts raw dicom file to nii and put it into BIDS format
				bids_neuron=../BIDS/sub-SCN${idx}
				chmod 777 -R ../dicom/SCN${idx}
