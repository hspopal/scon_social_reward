...
### ANOVA
```{r}
task_peer_mod <- lmer(value ~ peer*time_point + (1 + peer*time_point | record_id),
                       data=pre_post_data_long_clean)
summary(task_peer_mod)
```
...
# Export
write.csv(participants_sim_learn, 'derivatives/participants-task-sim_learn.csv',
          row.names = FALSE)
```
...
## ANOVA
```{r}
task_group_mod <- lmer(value ~ peer*time_point*group + (1 + peer*time_point*group | record_id),
                       data=pre_post_data_long_clean)
summary(task_group_mod)
```
...
## ANOVA
```{r}
task_mod <- lmer(value ~ peer*time_point + (1 + peer*time_point | record_id),
                       data=pre_post_data_long_error)

task_error_mod <- lmer(value ~ peer*time_point*errors + (1 + peer*time_point*errors | record_id),
                       data=pre_post_data_long_error)
summary(task_error_mod)
```

```{r}
anova(task_mod, task_error_mod, test="Chisq")
```
