...
```{r}
pre_post_data <- read.csv('derivatives/behavioral/SCONNPreAndPostTest-PreAndPostData_DATA_2024-06-07_1515.csv')

#pre_post_data <- subset(pre_post_data, 'SCN' %in% record_id)
...
presim1: How much are you interested in learning about Similar Peer?
predis1: How much are you interested in learning about Dissimilar Peer?
presim2: How much do you think you would want to spend time with Similar Peer if you knew them in real life?
predis2: How much do you think you would want to spend time with Dissimilar Peer if you knew them in real life?
presim3: How similar do you think you are to Similar Peer?
presim3: How similar do you think you are to Dissimilar Peer?
postsim1: How do you feel about Similar Peer?
postdis1: How do you feel about Dissimilar Peer?
postsim2: How much do you have in common with Similar Peer?
postdis2: How much do you have in common with Dissimilar Peer?
postsim3: How much would you want to spend time with Similar Peer if you knew them in real life?
postdis3: How much would you want to spend time with Dissimilar Peer if you knew them in real life?
postsim4: How much did you want to learn about Similar Peer?
postdis4: How much did you want to learn about Dissimilar Peer?
postcomp: How much did you want to learn about the computer?
postwhomost: Who or what did you like learning about the most?



```{r}
temp_df <- subset(pre_post_data_long, question %in% c('postsim1','postdis1'))

plot1 <- ggplot(temp_df, aes(x=value, color=question)) + geom_bar(stat='mean') + theme_classic()

plot2 <- ggplot(temp_df, aes(x=value, hue=question)) + geom_histogram() + theme_classic()
...
for (n in 1:nrow(pre_post_data_long)) {
  ...
  if (grepl( 'sim', pre_post_data_long$question[n], fixed = TRUE)) {
    pre_post_data_long$peer[n] <- 'sim'
  } else if (grepl( 'dis', pre_post_data_long$question[n], fixed = TRUE)) {
    pre_post_data_long$peer[n] <- 'dis'
  }
...
```{r}
ggplot(data = subset(pre_post_data_long, question %in% c('presim3', 'predis3', 
                                                         'postsim2','postdis3')), 
